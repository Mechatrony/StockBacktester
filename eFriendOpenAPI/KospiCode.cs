...
public class KospiCode {
  ...
  private static string[] part2Columns = [
  ...
  public static KospiCode ReadFromMstFile(string line) {
    ...
    int fieldTotalLength = fieldSpecs.Sum();
    string rf2 = line[^fieldTotalLength..];

    Type type = code.GetType();

    foreach (var field in fieldSpecs.Zip(part2Columns, (value, name) => (value, name))) {
  ...
  public override string ToString() {
    ...
    decimal 기준가 = this.기준가.ToDecimal();
    decimal ROE = this.ROE.ToDecimal();
    decimal 당기순이익 = this.당기순이익.ToDecimal();
    decimal 매출액 = this.매출액.ToDecimal();
    decimal 시가총액 = this.시가총액.ToDecimal();
